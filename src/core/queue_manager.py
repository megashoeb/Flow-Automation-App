...
class AsyncQueueManager(QThread):
    def __init__(self):
        ...
        self.account_parallel_slots = max(1, min(6, get_int_setting("slots_per_account", 3)))
        ...
        if gen_mode_raw not in {"browser_per_slot", "multi_tab"}:
    ...
    async def process_queue(self):
        if self.generation_mode == "multi_tab":
            self.signals.log_msg.emit("[SYSTEM] Generation mode: Multi-Tab (1 browser per account, N tabs)")
            try:
                from src.core.multitab_mode import MultiTabManager
                manager = MultiTabManager(self)
                await manager.run()
            except ImportError:
                self.signals.log_msg.emit("[ERROR] multitab_mode.py not found. Falling back to browser mode.")
            except Exception as e:
                self.signals.log_msg.emit(f"[ERROR] Multi-Tab failed: {str(e)[:100]}. Falling back to browser mode.")
            else:
                return
