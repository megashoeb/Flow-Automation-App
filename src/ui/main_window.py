...
class MainWindow(QMainWindow):
    ...
    def setup_settings(self):
        ...
        # ── Generation Mode (Browser vs Multi-Tab) ──
        ...
        self.cmb_generation_mode.addItem("Multi-Tab (experimental \u2014 low RAM)", "multi_tab")
        self.cmb_generation_mode.setToolTip(
            "Browser per slot: Each slot opens its own browser (~300MB each). Proven stable.\n\n"
            "Multi-Tab: ONE browser per account, each slot is a separate tab (~30MB each).\n"
            "Allows 20-40 parallel slots in ~1-1.5GB RAM. EXPERIMENTAL."
        )
